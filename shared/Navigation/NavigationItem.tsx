...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({
						...
						>
							...
							>
								<div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black dark:ring-white ring-opacity-5 dark:ring-opacity-10 text-sm">
									<div
										className={`relative bg-white dark:bg-gray-900 px-3 py-6 grid gap-1 grid-cols-${menu.megaMenu?.length}`}
									>
		...
		return (
			<li key={item.id}>
				<Link
					...
					className="inline-flex items-center py-1 px-2 rounded hover:text-neutral-700 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-neutral-200 font-normal text-neutral-6000 dark:text-neutral-300"
					href={item.href || ''}
				>
					{item.name}
				</Link>
				...
				{() => (
					<>
						...
						>
							...
							>
								<ul className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-white dark:ring-opacity-10 text-sm relative bg-white dark:bg-gray-900 py-4 grid space-y-1">
	...
	const renderDropdownMenuNavlink = (item: NavItemType) => {
		return (
			<Link
				...
				rel="noopener noreferrer"
				className="flex items-center font-normal text-neutral-6000 dark:text-neutral-300 py-2 px-4 rounded-md hover:text-neutral-700 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-neutral-200 "
				href={item.href || ''}
			>
				{item.name}
				{item.type && (
	...
	const renderMainItem = (item: NavItemType) => {
		return (
			<Link
				rel="noopener noreferrer"
				className="inline-flex items-center text-sm xl:text-base font-normal text-neutral-700 dark:text-neutral-300 py-2 px-4 xl:px-5 rounded-full hover:text-neutral-900 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-neutral-200"
				href={item.href || '/'}
			>
				{item.name}
				{item.type && (
