		...
		<Popover className="flex relative flex-1">
			{({ open, close }) => (
				<>
					...
					>
						<Popover.Panel className="absolute left-0 z-10 w-full sm:min-w-[340px] max-w-sm bg-white dark:bg-gray-800 top-full mt-3 py-5 sm:py-6 px-4 sm:px-8 rounded-3xl shadow-xl">
							<div className="">
									...
									{typeOfProperty.map((item, index) => (
										<div key={item.name} className="">
											<Checkbox
												...
												onChange={(e) => {
													const newState = typeOfProperty.map(
														(item, i) => {
															if (i === index) {
