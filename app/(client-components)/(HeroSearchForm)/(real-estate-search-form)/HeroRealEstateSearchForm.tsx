...
const HeroRealEstateSearchForm: FC<HeroRealEstateSearchFormProps> = ({
	...
	const [tabActive, setTabActive] =
		useState<SearchRealEstateTab>(currentTab);

	const renderTab = () => {
		return (
			<ul className="ml-6 md:ml-16 xl:ml-20 inline-flex space-x-4 sm:space-x-8 lg:space-x-10 bg-white dark:bg-gray-900 pb-6 md:p-6 !pl-0 xl:p-0 rounded-t-3xl">
				{tabs.map((tab) => {
					...
					return (
						...
						>
							{active && (
								<span className="block w-2.5 h-2.5 rounded-full bg-gray-800 dark:bg-gray-100 mr-2" />
							)}
							<span>{tab}</span>
						</li>
					);
